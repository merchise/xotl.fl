...
from xotl.fl.typecheck import EMPTY_TYPE_ENV, sidentity, typecheck, unify
...
def test_combinators():
    ...
    _phi, t = typecheck(K, EMPTY_TYPE_ENV)
...
def test_basic_builtin_types():
    ...
    _phi, t = typecheck(parse_expression("not True"), builtins_env)
    assert t == BoolType
    _phi, t = typecheck(parse_expression("not False"), builtins_env)
    assert t == BoolType

    userfuncs = {"toString": TypeScheme.from_str("a -> [Char]")}
    _phi, t = typecheck(parse_expression("either toString id"), dict(builtins_env, **userfuncs))
    assert len(find_tvars(t)) == 1
    unify(Type.from_str("Either a [Char] -> [Char]"), t)


def test_composition():
    _phi, t = typecheck(parse_expression("let id x = x in id . id"), builtins_env)
    unify(Type.from_str("a -> a"), t)
    unify(Type.from_str("(a -> a) -> (a -> a)"), t)

    _phi, t = typecheck(parse_expression("Left . Right"), builtins_env)
...
def test_regression_missing_dynamic_builtins():
    _phi, t = typecheck(parse_expression("let pair x y = (x, y) in pair 1 2"))
