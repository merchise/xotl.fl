...
def p_constrained_type_expr_bare(prod):
    ...
    def isvalid(constraint):
        return (
            isinstance(constraint, TypeCons)
            and len(constraint.subtypes) == 1
            and isinstance(constraint.subtypes[0], TypeVariable)
        )
...
def p_instance(prod):
    ...
    if isinstance(type_, TypeCons) and len(type_.subtypes) == 1:
        ...
        if is_simple_type(t):
            ...
            return
    raise ParserError(f"Invalid instance definition: {constraints} => {type_}")
...
type_parser = yacc.yacc(debug=False, start="st_type_expr", tabmodule="type_parsertab")

expr_parser = yacc.yacc(debug=False, start="st_expr", tabmodule="expr_parsertab")

program_parser = yacc.yacc(debug=True, start="program", tabmodule="program_parsertab")
